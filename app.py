...
from typing import List, Dict, Any, Tuple, Optional# app.py — Tasador Agrícola Noroeste (VERSIÓN INTEGRAL COMPROBADA CON SHEETS)
import streamlit as st
import os
import io
import re
...
# ------------------------------------------------------------
# VISTA ACCESO (Desactivado bypass para evitar colgar de Drive)
# ------------------------------------------------------------
def vista_acceso():
    st.session_state["logged_in"] = True
    st.session_state["vendedor"] = "Tasador General"
    st.rerun()
...
# 🛠️ IGNORAMOS CUALQUER CHORIZO DE DRIVE VIEJO PARA QUE NO CRASHEE
COEFS = DEFAULT_COEFS

# 🛠️ INICIALIZACIÓN POR ENTORNO MODERNO (Inyectamos la API Key en las variables del sistema de Google)
if "vertex_client" not in st.session_state:
    if "GEMINI_API_KEY" in st.secrets:
        os.environ["GEMINI_API_KEY"] = st.secrets["GEMINI_API_KEY"]
        os.environ["GOOGLE_API_KEY"] = st.secrets["GEMINI_API_KEY"]
    # Llamamos sin pasar diccionarios PEM rotos
    st.session_state.vertex_client = ia_engine.conectar_vertex(None)

# GPS
texto_ubicacion = "UBICACIÓN EXTERNA"
            ...
            if err:
                ...
                with st.spinner("Procesando con Gemini..."):
                    try:
                        ...
                        # --- GUARDAR EN GOOGLE SHEETS EXCEL ---
                        try:
...
# 🛠️ PARCHE INTEGRAL V2: Simulamos el diccionario completo para contentar al .get() de ia_engine.py
if "vertex_client" not in st.session_state:
    if "GEMINI_API_KEY" in st.secrets:
        credenciales_ficticias = {
            "private_key": st.secrets["GEMINI_API_KEY"],
            "project_id": "subida-fotos-drive"
        }
        st.session_state.vertex_client = ia_engine.conectar_vertex(credenciales_ficticias)
    else:
